...
public sealed class EmissionsTests
{
    ...
    private void AssertEmissionSetIsEqual(Emissions[] expectedEmissions, Emissions?[] emissions)
    {
        ...
        for (var i = 0; i < expectedEmissions.Length; i++)
        {
            ...
            // Console.WriteLine($"Expected: EngineType: {expectedEmissions.ElementAt(i).engine_type} Date: {expectedEmissions.ElementAt(i).date} Actual: EmissionsType: {emissions.ElementAt(i).engine_type} Date: {emissions.ElementAt(i).date}");
            // Console.WriteLine($"Expected: Trips: {expectedEmissions.ElementAt(i).trips} Actual: Trips: {emissions.ElementAt(i).trips}");
